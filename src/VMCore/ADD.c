...
extern uint64 __VAR__ add_asm(uint64 __VAR__ add1, uint64 __VAR__ add2, uint64 __PTR__ flag);

//
// ADDָ���������
// ����ִ��״̬��־
// 
byte __VAR__ ADD(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne && null != instruct->_OPTwo)
	{
		...
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_REG__:
			...
			case __INSTRUCTION_SECOND_OPTYPE_REG__:
				cpu->GREG[instruct->_OPOne->_Reg_Expand] = add_asm
				(
