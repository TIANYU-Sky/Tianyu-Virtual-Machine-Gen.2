...
extern uint64 __VAR__ sub_asm(uint64 __VAR__ minuend, uint64 __VAR__ subtr, uint64 __PTR__ flag);

//
// SUB指令操作方法
// 返回执行状态标志
// 
byte __VAR__ SUB(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
		...
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_REG__:
			...
			case __INSTRUCTION_SECOND_OPTYPE_REG__:
				if (instruct->_OPDirect)
					cpu->GREG[instruct->_OPTwo->_Reg_Imm] = sub_asm
					(
					...
					cpu->GREG[instruct->_OPOne->_Reg_Expand] = sub_asm
					(
						cpu->GREG[instruct->_OPOne->_Reg_Expand],
						cpu->GREG[instruct->_OPTwo->_Reg_Imm],
						__V_To_P__ cpu->FLAGREG
