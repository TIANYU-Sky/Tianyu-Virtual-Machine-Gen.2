...
// 返回执行状态标志
// 
byte __VAR__ JC(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
...
// 返回执行状态标志
// 
byte __VAR__ JNC(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne)
	{
		if (!(cpu->FLAGREG & __TYVM_SIGNREGISTER_CARRY_FLAG__))
