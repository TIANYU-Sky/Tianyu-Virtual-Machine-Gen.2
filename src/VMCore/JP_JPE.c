...
// 返回执行状态标志
// 
byte __VAR__ JP_JPE(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne)
	{
		if (cpu->FLAGREG & __TYVM_SIGNREGISTER_PARITY_FLAG__)
...
// 返回执行状态标志
// 
byte __VAR__ JPO_JNP(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne)
	{
		if (!(cpu->FLAGREG & __TYVM_SIGNREGISTER_PARITY_FLAG__))
