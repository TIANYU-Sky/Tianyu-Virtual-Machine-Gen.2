...
// 返回执行状态标志
// 
byte __VAR__ LCS(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne)
	{
		...
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_REG__:
			cpu->CS = cpu->GREG[instruct->_OPOne->_Reg_Expand];
			break;
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_IMM__:
			cpu->CS = instruct->_OPOne->_Imm_Offset;
			break;
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_MEM__:
		{
			...
			cpu->CS = BytesToUint64((byte __PTR__)mem, instruct->_OPNumber);
			if (instruct->_Lock)
