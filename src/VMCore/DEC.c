...
extern uint64 __VAR__ dec_asm(uint64 __VAR__ source, uint64 __PTR__ flag);

byte __VAR__ DEC(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne)
	{
		...
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_MEM__:
		{
			...
			data = dec_asm(data, __V_To_P__ cpu->FLAGREG);
