...
extern uint64 __VAR__ shl_asm(uint64 __VAR__ op1, uint64 __VAR__ op2, uint64 __PTR__ flag);
extern uint64 __VAR__ sal_asm(uint64 __VAR__ op1, uint64 __VAR__ op2, uint64 __PTR__ flag);

//
// SHL指令操作方法
// 返回执行状态标志
// 
byte __VAR__ SHL(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne && null != instruct->_OPTwo)
	{
		...
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_REG__:
			...
			case __INSTRUCTION_SECOND_OPTYPE_REG__:
				cpu->GREG[instruct->_OPOne->_Reg_Expand] = shl_asm
				(
...
// 返回执行状态标志
// 
byte __VAR__ SAL(struct instructionpackage __PTR__ instruct, CPU __PTR__ cpu, bool __PTR__ excuter_flag)
{
	if (null != instruct->_OPOne && null != instruct->_OPTwo)
	{
		switch (instruct->_OPOne->_Addressing)
		...
		case __INSTRUCTION_PACKAGE_OPONE_ADDRESSING_REG__:
			...
			{
			case __INSTRUCTION_SECOND_OPTYPE_REG__:
				cpu->GREG[instruct->_OPOne->_Reg_Expand] = sal_asm
				(
					cpu->GREG[instruct->_OPOne->_Reg_Expand],
					cpu->GREG[cpu, instruct->_OPTwo->_Reg_Imm],
					__V_To_P__ cpu->FLAGREG
